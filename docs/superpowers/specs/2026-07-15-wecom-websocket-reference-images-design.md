...
接收处理器先尝试解析并完成匹配的 Future。消息回调和事件处理通过受跟踪的后台任务执行，保证接收循环不会被 RAG 生成和出站 ACK 等待阻塞。WebSocket 断开或 Channel 停止时，所有等待中的 Future 都会以异常结束并被清理；Channel 停止时还会取消并回收回调任务。
...
2. 对每个 Base64 编码的分片发送 `aibot_upload_media_chunk`，分片编号从 0 开始；单片不超过 512 KiB，最多 100 片。
...
- 非对象 JSON、非法响应头和缺失或非数字 `errcode` 不会破坏接收循环。
...
- 接收消息回调不阻塞后续 ACK 的读取，以及畸形响应的隔离处理。
